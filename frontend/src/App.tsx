...
type Provider = { id: string; label: string; adapter?: string; configured: boolean; default_base_url?: string; default_api_key_env?: string; model_examples: string[] };
...
function App() {
  ...
  const [youtubeStrategy, setYoutubeStrategy] = useState("automatic");
  ...
  const baseUrlEditable = selectedProvider?.adapter === "openai_compatible" || llmProvider === "ollama";
  const providerNeedsKey = llmEnabled && !["none", "ollama"].includes(llmProvider);
  ...
  async function submit() {
    ...
    const options = {
      ...
      speech: { provider: speech, model: speechModel, language: inputLanguage === "auto" ? null : inputLanguage, remote_base_url: speechBaseUrl || null, api_key: speechApiKey || null },
      youtube: { transcript_strategy: youtubeStrategy },
      ocr: { language: ocrLanguage },
      llm: { enabled: llmEnabled, provider: llmProvider, model: llmModel || null, base_url: llmBaseUrl || null, api_key: llmApiKey || null, output_language: outputLanguage, vision_enabled: llmVision },
      output: { pdf: outputPdf, csv: outputCsv, word: outputWord, transcript: outputTranscript },
      review: { gate_before_artifacts: reviewGate },
    };
    if (speech === "openai_compatible" && (!speechBaseUrl.trim() || !speechApiKey.trim())) { setError("Enter the remote speech base URL and API key."); setBusy(false); return; }
    if (llmEnabled && providerNeedsKey && !llmApiKey.trim()) { setError("Enter an API key for the selected provider."); setBusy(false); return; }
    if (llmEnabled && llmProvider === "openai_compatible" && !llmBaseUrl.trim()) { setError("Enter a base URL for the OpenAI-compatible provider."); setBusy(false); return; }
    try {
      ...
      setJob(body); setLlmApiKey(""); setSpeechApiKey(""); setEvents([`Job ${body.job_id} queued`]);
    } catch (reason) {
  ...
  return <main className="shell">
    ...
    <section className="workspace-grid">
      <div className="panel intake-panel">
        ...
        <div className="control-row"><label>PROFILE<select value={profile} onChange={(event) => setProfile(event.target.value)}>{(settings?.profiles || fallbackProfiles).map((item) => <option value={item.id} key={item.id}>{item.label}</option>)}</select></label><label>SPEECH<select value={speech} onChange={(event) => { setSpeech(event.target.value); if (event.target.value !== "openai_compatible") setSpeechApiKey(""); }}><option value="auto">Automatic</option><option value="none">Captions only</option><option value="faster_whisper">Local Whisper</option><option value="openai_compatible">Remote API</option></select></label></div>
        ...
        {advancedOpen && <div className="advanced-panel">
          ...
          <div className="settings-section"><div className="settings-title">YOUTUBE TRANSCRIPT</div><label>STRATEGY<select value={youtubeStrategy} onChange={(event) => setYoutubeStrategy(event.target.value)}><option value="automatic">Automatic: transcript first</option><option value="transcript_first">Transcript service first</option><option value="ytdlp_first">yt-dlp captions first</option><option value="whisper_first">Local Whisper first</option><option value="captions_only">Captions only</option></select></label><small className="settings-description">Automatic avoids coupling caption rate limits to media download and caches successful transcripts.</small></div>
          {speech === "openai_compatible" && <div className="settings-section"><div className="settings-title">REMOTE SPEECH PROVIDER</div><label>MODEL<input value={speechModel} onChange={(event) => setSpeechModel(event.target.value)} placeholder="whisper-1" /></label><div className="control-row"><label>BASE URL<input value={speechBaseUrl} onChange={(event) => setSpeechBaseUrl(event.target.value)} placeholder="https://api.example.com/v1" /></label><label>API KEY<input type="password" autoComplete="new-password" value={speechApiKey} onChange={(event) => setSpeechApiKey(event.target.value)} placeholder="Enter key for this job" /></label></div><small className="settings-description">Used for this job only and never written to artifacts.</small></div>}
          ...
          <div className="settings-section"><div className="settings-title">LLM ENRICHMENT</div><label className="toggle-row"><span>Enable LLM tasks and explanations</span><input type="checkbox" checked={llmEnabled} onChange={(event) => { const enabled = event.target.checked; setLlmEnabled(enabled); if (!enabled) { setLlmProvider("none"); setLlmApiKey(""); } else if (llmProvider === "none") setLlmProvider(settings?.llm.find((item) => item.id !== "none")?.id || "gemini"); }} /></label><label>PROVIDER<select value={llmProvider} onChange={(event) => { setLlmProvider(event.target.value); setLlmApiKey(""); }} disabled={!llmEnabled}>{(settings?.llm || [{ id: "none", label: "No LLM" }]).map((item) => <option value={item.id} key={item.id}>{item.label}</option>)}</select></label><label>MODEL<input list="llm-models" value={llmModel} disabled={!llmEnabled} onChange={(event) => setLlmModel(event.target.value)} placeholder="Provider model name" /><datalist id="llm-models">{selectedProvider?.model_examples.map((model) => <option value={model} key={model} />)}</datalist></label><div className="control-row"><label>BASE URL<input value={llmBaseUrl} disabled={!llmEnabled || !baseUrlEditable} onChange={(event) => setLlmBaseUrl(event.target.value)} placeholder="Optional custom endpoint" /></label>{!["none", "ollama"].includes(llmProvider) && <label>API KEY<input type="password" autoComplete="new-password" value={llmApiKey} disabled={!llmEnabled} onChange={(event) => setLlmApiKey(event.target.value)} placeholder="Enter key for this job" /></label>}</div><label className="toggle-row"><span>Send low-confidence frames for visual analysis</span><input type="checkbox" checked={llmVision} disabled={!llmEnabled} onChange={(event) => setLlmVision(event.target.checked)} /></label>{llmEnabled && selectedProvider && <small className={providerNeedsKey && !llmApiKey ? "provider-missing" : "provider-ready"}>{providerNeedsKey ? (llmApiKey ? "Key will be used for this job only and not stored." : "Enter an API key for this job.") : "This provider does not require an API key."}</small>}</div>
        ...
        <button className="primary-button" disabled={!hasInput || busy} onClick={submit}>{busy ? "Starting…" : "Process source"}<span>→</span></button><p className="microcopy">No LLM required · Runtime keys are used for this job only · Resumable artifacts</p>
      </div>
