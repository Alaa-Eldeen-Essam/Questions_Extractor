...
type Extraction = { questions?: Question[]; transcript?: { segments?: { text: string; start_seconds: number }[] }; ocr?: { text: string; frame: { timestamp_seconds: number; path: string } }[] };
...
function App() {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/jobs/${job.job_id}/artifacts/extraction.json`).then((response) => response.ok && response.json()).then((value) => value && setData(value));
  }, [job?.job_id, job?.status]);
  ...
  async function submit() {
    ...
    try {
      ...
      if (file) {
        ...
        response = await fetch(`${API}/api/jobs`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ source, profile, options: { speech: { provider: speech } } }) });
      }
  ...
  const visibleStages = useMemo(() => stages.map((name) => ({ name, status: job?.stages?.[name]?.status || (job?.status === "queued" ? "pending" : "pending") })), [job]);
  ...
  return <main className="shell">
    ...
    <section className="workspace-grid">
      <div className="panel intake-panel"><div className="panel-heading"><div><span className="section-number">01</span><h2>Bring your material</h2></div><span className="status-dot">LOCAL-FIRST</span></div><div className="input-tabs"><button className={!file ? "active" : ""} onClick={() => setFile(null)}>Video link</button><button className={file ? "active" : ""} onClick={() => document.getElementById("file-input")?.click()}>Upload file</button></div><input id="file-input" className="hidden" type="file" accept="video/*,audio/*,.pdf" onChange={(event) => { setFile(event.target.files?.[0] || null); setSource(""); }} /><div className="source-input"><span>↗</span><input aria-label="YouTube URL" placeholder="Paste a YouTube URL or local path" value={source} disabled={Boolean(file)} onChange={(event) => setSource(event.target.value)} />{file && <strong>{file.name}</strong>}</div><div className="control-row"><label>PROFILE<select value={profile} onChange={(event) => setProfile(event.target.value)}><option value="fast">Fast</option><option value="balanced">Balanced</option><option value="accurate">High accuracy</option></select></label><label>SPEECH<select value={speech} onChange={(event) => setSpeech(event.target.value)}><option value="auto">Automatic</option><option value="none">Captions only</option><option value="faster_whisper">Local Whisper</option><option value="openai_compatible">Remote API</option></select></label></div><button className="primary-button" disabled={!hasInput || busy} onClick={submit}>{busy ? "Starting…" : "Extract study material"}<span>→</span></button><p className="microcopy">No LLM required · Your keys stay on the server · Resumable artifacts</p></div>
